...
try:
    ...
    c = conn.cursor()
    # Create table
    c.execute("CREATE TABLE Weather_data(DeviceId real,AirTemperature real, CloudCover real,Humidity real,AirPressure real,EventDateTime real, Visibility real,WindDirection real, WindSpeed real)")
    conn.commit()
    conn.close()
except Exception as e:
    logging.error(e, exc_info=True)
...
def insert_into_weather(DeviceId,AirTemperature,CloudCover,EventDateTime,Humidity,AirPressure,Visibility,WindDirection,WindSpeed):
    try:
        conn = sqlite3.connect('Weather.db')
        c = conn.cursor()
        c.execute("UPDATE Weather_data SET EventDateTime='%s',AirTemperature=%d,CloudCover=%d,Humidity=%d,AirPressure=%d,Visibility=%d,WindDirection=%d,WindSpeed=%d WHERE DeviceId=%d"%(EventDateTime,AirTemperature,CloudCover,Humidity,AirPressure,Visibility,WindDirection,WindSpeed,DeviceId))
        conn.commit()
        c = conn.cursor()
        ...
        if no_of_update==0:
            c = conn.cursor()
            c.execute("INSERT INTO Weather_data VALUES (%d,%d,%d,%d,%d,'%s',%d,%d,%d)" %(DeviceId,AirTemperature, CloudCover,Humidity,AirPressure,EventDateTime, Visibility,WindDirection,WindSpeed))
            conn.commit()
            conn.close()
        else:
            conn.close()
...
try:
    ...
    #latlogList = [(28.7041,77.1025)]
    LatLongDeviceMap = dict(zip(df.new_col, df.EntryId))
    #LatLongDeviceMap = {(28.7041,77.1025):1}
...
def SendEvent():   
    ...
    for k in latlogList:
        ...
        for index, row in df.iterrows():
            ...
            if count == 0:
                try:
                    insert_into_weather(DeviceId,AirTemperature,CloudCover,EventDateTime,Humidity,AirPressure,Visibility,WindDirection,WindSpeed)
                except:
                    pass
                count+=1
